...
public void ShowRankTopMenu(int client)
{
	...
	{	
		...
		Format(szValue, sizeof(szValue), " Your rank: [NEW]\n");

	Format(szOption,sizeof(szOption),"%s Points: %d\n KD: %d:%d \n Time Played: %d mins",szValue
	...
	hMenu.AddItem("","Top 10 Ranked");

	Format(szOption,sizeof(szOption),"Top 20 Jumps");
...
public void SQLQueryCallback_sql_SRC_REC(Database hDatabase, DBResultSet hResults, const char[] sError, any data) 
{
	...
	hMenu.SetTitle("Top 10\n    RanK    Points           Kills         Deaths      Time        Name");
	hMenu.Pagination = 4;
