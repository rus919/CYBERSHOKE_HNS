...
public Action FirstCountdownMessage(Handle hTimer, any iClient)
{
    ...
    {
        PrintToChat(iClient, "\n  \x04CYBERSHOKE \x01| \x01%t", "Start Countdown", iCountdownTimeFloor, (iCountdownTimeFloor == 1) ? "" : "s");
    }
    ...
    if(g_iCountdownCount < g_fCountdownTime) {
        for(int iClient = 1; iClient < MaxClients; iClient++) {
            if(IsValidClient(iClient)) {
                ...
                if(iTimeDelta <= 3)
                    PrintToChat(iClient, "\n  \x04CYBERSHOKE \x01| \x01%t", "Start Countdown", iTimeDelta, (iTimeDelta == 1) ? "" : "s");
        ...
        for(int iClient = 1; iClient < MaxClients; iClient++) {
            if(IsValidClient(iClient)){
                PrintToChat(iClient, "\n  \x04CYBERSHOKE \x01| \x01%t", "Round Start");
                //PrintCenterText(iClient, "\n  %t", "Round Start");
            }
        }
...
public Action Command_ToggleKnife(int iClient, int args)
{
    if(iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient)) {
        ...
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", g_baToggleKnife[iClient] ? "Toggle Knife On" : "Toggle Knife Off");
        ...
        if(IsClientInGame(iTarget)) {
            ...
            if(iClientTeam != iTargetTeam && iTargetTeam != CS_TEAM_SPECTATOR) {
                ...
                if(GetVectorDistance(faTargetCoord, faClientCoord) <= 500) {
                    PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Respawn Aborted Enemy Near");
                    return false;
                }
            }
        }
...
public Action Command_Respawn(int iClient, int args)
{
    if(iClient > 0 && iClient <= MaxClients && IsClientInGame(iClient)) {
        if(!g_bRespawnMode)
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Respawn Aborted Off");
        else if(IsPlayerRespawning(iClient))
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Respawn Aborted Alive");
        else if(!(GetEntityFlags(iClient) & FL_ONGROUND))
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Respawn Aborted In Flight");
        else {
...
public void MakeClientInvisible(int iClient, float fDuration)
{
    ...
    PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invisible On", fDuration);
...
public Action MakeClientVisible(Handle hTimer, any iClient)
{
    ...
    if(IsPlayerAlive(iClient))
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invisible Off");
}
...
public void BreakInvisibility(int iClient, int iReason)
{
    if(g_haInvisible[iClient] != INVALID_HANDLE) {
        ...
        if(iReason == REASON_ENEMY_TOO_CLOSE)
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invisibility Broken Enemy Near");
        else if(iReason == REASON_LADDER)
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invisibility Broken Climb");
        else if(iReason == REASON_GRENADE)
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invisibility Broken Threw Grenade");
    }
}
...
public Action HandlePlayerSpectateRequest(int iClient) {
    ...
    if(g_bRespawnMode) {
        if(iTeam == CS_TEAM_T)
            if(PlayerCanDisappear(iClient)) {
                PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Spectate Pending", 5.0, "s");
        ...
        if(iTeam == CS_TEAM_CT || CS_TEAM_T) {
            if(IsPlayerAlive(iClient)) {
                PrintToConsole(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Spectate Deny Alive");
                return Plugin_Stop;
            }
            else {
...
public Action Command_JoinTeam(int iClient, const char[] sCommand, int iArgCount)
{
    ...
    if(iTeam == CS_TEAM_T || iTeam == CS_TEAM_CT) {
        if(iChosenTeam == JOINTEAM_T || iChosenTeam == JOINTEAM_CT || iChosenTeam == JOINTEAM_RND) {
            if(IsPlayerAlive(iClient)) {
                PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Team Change Deny Alive");
                return Plugin_Stop;
            }
                ...
                PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Team Change Deny Balanced");
                return Plugin_Stop;
            }
        }
            ...
            PrintToConsole(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Invalid Team Console", iClient);
            return Plugin_Stop;
        }
    }
        ...
        if(iDelta > iLimitTeams || -iDelta > iLimitTeams) {
            ...
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", (g_iaInitialTeamTrack[iClient] == CS_TEAM_T) ? "Assigned To Team T" : "Assigned To Team CT");
...
public Action Command_Kill(int iClient, const char[] sCommand, int iArgCount)
{
    ...
    if (!g_bBlockConsoleKill || iClient == 0 || iClient > MaxClients)
        return Plugin_Continue;
    PrintToConsole(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Kill Deny", iClient);
    return Plugin_Stop;
}
        ...
        if(iVictimTeam == CS_TEAM_T) {
            ...
            if(iAttacker > 0 && iAttacker <= MaxClients) {
                ...
                if(iAttackerTeam == CS_TEAM_CT) {
                    ...
                    if(!g_bRespawnMode)
                        PrintToChat(iAttacker, "  \x04CYBERSHOKE \x01| \x01%t", 
                ...
                if(g_iSuicidePointsPenalty) {
                    ...
                    PrintToChat(iVictim, "  \x04CYBERSHOKE \x01| \x01%t", "Died By Falling", g_iSuicidePointsPenalty);
                }
                if(g_bRespawnMode) {
                    if(!g_baDiedBecauseRespawning[iVictim]) {
                        ...
                        if(iVictimTeamBalanceStatus & (TEAM_NEEDS_NERF | TEAM_CAN_TAKE_NERF)) {
                            ...
                            PrintToChat(iVictim, "  \x04CYBERSHOKE \x01| \x01%t", "Assigned To Team CT");
                        }
                    }
                    else {
                        g_baAvailableToSwap[iVictim] = false;
                ...
                if(g_bRespawnMode) {
                    if(!g_baDiedBecauseRespawning[iVictim]) {
                        ...
                        if(iVictimTeamBalanceStatus & TEAM_NEEDS_NERF) {
                            ...
                            CS_SwitchTeam(iAttacker, CS_TEAM_T);
                            g_iaInitialTeamTrack[iAttacker] = CS_TEAM_T;
                            PrintToChat(iVictim, "  \x04CYBERSHOKE \x01| \x01%t", "Assigned To Team T");
                        }
                        else {
                            g_baAvailableToSwap[iVictim] = false;
                        }
            ...
            if(!IsPlayerAlive(iTarget)) {
                ...
                if((iClientTeam == CS_TEAM_CT && iTargetTeam == CS_TEAM_T) || (iClientTeam == CS_TEAM_T && iTargetTeam == CS_TEAM_CT))
                    if(g_baAvailableToSwap[iTarget]) {
                        ...
                        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Swapped", sNickname);

                        GetClientName(iClient, sNickname, sizeof(sNickname));
                        PrintToChat(iTarget, "  \x04CYBERSHOKE \x01| \x01%t", "Swapped", sNickname);
                    }
            }
    }
}
...
public void OnRoundEnd(Event hEvent, const char[] name, bool dontBroadcast)
{
    ...
    if(iWinningTeam == CS_TEAM_T) {
        if(!g_iMaximumWinStreak || ++g_iTWinsInARow < g_iMaximumWinStreak)
            PrintToChatAll("  \x04CYBERSHOKE \x01| \x01%t", "T Win");
        else {
            SwapTeams();
            g_iTWinsInARow = 0;
            //Set the team scores
            ...
            if(g_iMaximumWinStreak)
                PrintToChatAll("  \x04CYBERSHOKE \x01| \x01%t", "T Win Team Swap");
        }
    ...
    else if(iWinningTeam == CS_TEAM_CT)
    {
        ...
        SwapTeams();
        PrintToChatAll("  \x04CYBERSHOKE \x01| \x01%t", "CT Win");
        g_iTWinsInARow = 0;
        //Set the team scores
...
stock void GiveGrenades(int iClient)
{
    ...
    if(iLastType == -1)
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "No Grenades");
    else {
        ...
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Grenades Received", sGrenadeMessage);
    }
}
...
stock void Freeze(int iClient, float fDuration, int iType, int iAttacker = 0)
{
    ...
    if(iAttacker == 0) {
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Frozen", fDuration);
    }
    else if(iAttacker == iClient) {
        PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Frozen Yourself", fDuration);
    }
    else if(iAttacker <= MaxClients) {
        if(IsClientInGame(iAttacker)) {
            char sAttackerName[MAX_NAME_LENGTH];
            GetClientName(iAttacker, sAttackerName, sizeof(sAttackerName));
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Frozen By", sAttackerName, fDuration);
        }
    }
    if(g_baFrozen[iClient]) {
        if(g_haFreezeTimer[iClient] != INVALID_HANDLE) {
...
public Action Unfreeze(Handle hTimer, any iClient)
{
    if(iClient && IsClientInGame(iClient)) {
        if(g_baFrozen[iClient]) {
            ...
            if(IsPlayerAlive(iClient))
                PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Unfreeze");
        }
    }
    return Plugin_Continue;
}
...
public Action UnfreezeCountdown(Handle hTimer, any iClient)
{
    if(iClient && IsClientInGame(iClient)) {
        ...
        if(IsPlayerAlive(iClient) && !g_bRespawnMode)
            PrintToChat(iClient, "  \x04CYBERSHOKE \x01| \x01%t", "Round Start");
    }
    return Plugin_Continue;
}
