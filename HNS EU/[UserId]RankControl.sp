		...
		if(g_Mytime-g_iOwnerCount[client][target]>=2)
		{
			...
			if (g_bSwithRound)
			{
				...
				if(GetEntityMoveType(client) != MOVETYPE_LADDER && GetEntityMoveType(target) != MOVETYPE_LADDER){
					...
					PrintToChat(target,"\x01\04 \x04CYBERSHOKE \x01| \x01You have lost  \x07%d points (you have: \x02%d) for \x04 Head Own",5,g_aPlayerStats[target][POINTS]);
...
static void Event_PlayerDeath(Handle event, const char[] sEvName, bool bDontBroadcast)   
{
	...
	if(g_bSwithRound)
	{
		if(client==attacker)
		{
			...
			PrintToChat(client,"\x01\04 \x04CYBERSHOKE \x01| \x01You have lost  \x07%d points (you have: \x02%d) \x01for \x04 suicide", 3, g_aPlayerStats[client][POINTS]);	
		}
		else if(attacker==0)
		{
			g_aPlayerStats[client][DEATH]++;
			...
			PrintToChat(client,"\x01\04 \x03[\x07RC\x03] \x01You have lost  \x07%d points (you have:: \x02%d) \x01for \x04 falling",PIE,g_aPlayerStats[client][POINTS]);
				...
				if(g_MatrixAssists[client][i] && attacker!=i && IsValidClient(i))
			{
				...
				PrintToChat(i,"\x01\04 \x04CYBERSHOKE \x01| \x01You earned  \x07%d points (you have: \x02%d) \x01for \x04 assist",PIE,g_aPlayerStats[i][POINTS]);
				g_Murder[i]++;
			}	
		}
		else if(attacker!=0 && client!=0)
		{
			...
			PrintToChat(client,"\x01\04 \x04CYBERSHOKE \x01| \x01You have lost  \x07%d points (you have: \x02%d) \x01for \x04 death",PieClient,g_aPlayerStats[client][POINTS]);
			PrintToChat(attacker,"\x01\04 \x04CYBERSHOKE \x01| \x01You earned  \x07%d points (you have: \x02%d) \x01for \x04 killing",PieAttacer,g_aPlayerStats[attacker][POINTS]);
			for(int i=1;i<=MaxClients;i++)
				if(g_MatrixAssists[client][i] && attacker!=i && IsValidClient(i))
			{
				g_aPlayerStats[i][ASSISTS]++;
		...
		if(IsValidClient(client))
	{
		if(g_iClientUserId[client])
		{
			...
			{	
				...
				PointsOperation(client,PIE);
				PrintToChat(client,"\x01\04 \x04CYBERSHOKE \x01| \x01You earned  \x07%d points (you have: \x02%d) \x01for \x04 winning the round",PIE,g_aPlayerStats[client][POINTS]);
			}
			if(GetClientTeam(client)==3 && g_bSwithPlayer[client])
			{
				int PIE=RoundLost(client);
				PointsOperation(client,-PIE);
				PrintToChat(client,"\x01\04 \x04CYBERSHOKE \x01| \x01You have lost  \x07%d points (you have: \x02%d) \x01for \x04for losing",PIE,g_aPlayerStats[client][POINTS]);
			}
			g_bSwithPlayer[client]=false;	
		}
