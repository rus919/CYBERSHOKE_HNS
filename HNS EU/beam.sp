...
public Action Beam_Main(int client) {
	...
	FormatEx(szBuffer, 128, "Beam %s", g_bBeam[client] ? "[ON]" : "[OFF]");
	menu.AddItem("0", szBuffer, ITEMDRAW_DEFAULT);

	FormatEx(szBuffer, 128, "Show CTRL %s", g_bShowDucks[client] ? "[ON]" : "[OFF]");
	menu.AddItem("1", szBuffer, g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	FormatEx(szBuffer, 128, "Beam in air %s", g_bAirFollow[client] ? "[ON]" : "[OFF]");
	menu.AddItem("2", szBuffer, g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	FormatEx(szBuffer, 128, "Beam on ground %s", g_bGroundFollow[client] ? "[ON]" : "[OFF]");
	...
	if(g_bBeam[client])
		menu.AddItem("5", "Beam color in duck", g_bShowDucks[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
