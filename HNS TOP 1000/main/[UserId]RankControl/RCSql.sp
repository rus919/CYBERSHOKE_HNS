...
{
	if((g_aPlayerStats[client][RANK] > 1000 || g_aPlayerStats[client][POINTS] <= 20 ) && !GetAdminFlag(GetUserAdmin(client), Admin_Ban, Access_Effective))
	{	
		char RankString[32];
		if(g_aPlayerStats[client][POINTS]>20)
		{
			...
			{
				Format(RankString, sizeof(RankString), "%dk+", g_aPlayerStats[client][RANK]/1000);
			}
			else
			{
				Format(RankString, sizeof(RankString), "%d", g_aPlayerStats[client][RANK]);
			}
		}	
		else
		{
			Format(RankString, sizeof(RankString), "NEW");
		}

		KickClient(client, "[CYBERSHOKE] HNS | Текущий ранг: %s\n\nСервер доступен только для игроков до 1000 ранга\n\nVK: vk.com/cybershoke | Discord: discord.io/cybershoke\n", RankString);
	}
}
