...
#define PREFIX "\x01\x04CYBERSHOKE \x01|"
#define MENUTITLE "[CYBERSHOKE] BEAM"
...
public Action Beam_Main(int client) {
	...
	FormatEx(szBuffer, 128, "Луч %s", g_bBeam[client] ? "[ВКЛ]" : "[ВЫКЛ]");
	menu.AddItem("0", szBuffer, ITEMDRAW_DEFAULT);

	FormatEx(szBuffer, 128, "Показ CTRL %s", g_bShowDucks[client] ? "[ВКЛ]" : "[ВЫКЛ]");
	menu.AddItem("1", szBuffer, g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	FormatEx(szBuffer, 128, "Луч в воздухе %s", g_bAirFollow[client] ? "[ВКЛ]" : "[ВЫКЛ]");
	menu.AddItem("2", szBuffer, g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	FormatEx(szBuffer, 128, "Луч на земле %s", g_bGroundFollow[client] ? "[ВКЛ]" : "[ВЫКЛ]");
	menu.AddItem("3", szBuffer, g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	menu.AddItem("4", "Цвет луча", g_bBeam[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);

	if(g_bBeam[client])
		menu.AddItem("5", "Цвет луча в приседании", g_bShowDucks[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	...
	if(action == MenuAction_End)
		...
		switch(select) {
			case 0: {
				...
				//PrintToChat(client, "%s Луч был %s\x01.", PREFIX, g_bBeam[client] ? "\x04Включен" : "\x02Выключен");
			...
			case 1: {
				...
				//PrintToChat(client, "%s Show ducks has been %s\x01.", PREFIX, g_bShowDucks[client] ? "\x04Включен" : "\x02Выключен");
