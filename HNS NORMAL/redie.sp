...
public Action:Event_Player_Death(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	{
		PrintToChat(client, "\x01[\x03Redie\x01] %t", "Dead message");
...
public Action:bringback(Handle:timer, any:client)
{
	...
	{
		PrintToChat(client, "\x01[\x03Redie\x01] %t", "You must be on a team");
	}
}
...
public Action:Command_Redie(client, args)
{
	if (!IsPlayerAlive(client))
	{
		...
		{
			PrintToChat(client, "\x01[\x03Redie\x01] %t", "New round");
		}
	}
	else
	{
		...
		{
			PrintToChat(client, "\x01[\x03Redie\x01] %t", "You must be dead to use redie");
		}
	}
	return Plugin_Handled;
}
...
public Action:advert(Handle:timer)
{
	...
	{
		//PrintToChatAll ("\x01[\x03Redie\x01] %t", "This server is running");
	}
	return Plugin_Continue;
}
