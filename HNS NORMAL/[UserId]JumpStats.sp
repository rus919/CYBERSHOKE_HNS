...
char sql_SJS_DELBYUSERID[JUMP_PATTERN][] = {
"DELETE FROM LJ WHERE user_id = %s",
"DELETE FROM BH WHERE user_id = %s",
"DELETE FROM MBH WHERE user_id = %s",
"DELETE FROM BCJ WHERE user_id = %s",
"DELETE FROM CJ WHERE user_id = %s",
"DELETE FROM CBHJ WHERE user_id = %s",
"DELETE FROM MCJ WHERE user_id = %s",
"DELETE FROM LAJ WHERE user_id = %s",
"DELETE FROM LABJ WHERE user_id = %s",
"DELETE FROM LACJ WHERE user_id = %s"};
...
#include "main\[UserId]JS\JSEnter.sp"
#include "main\[UserId]JS\JSSql.sp"
#include "main\[UserId]JS\JSMenu.sp"
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_delstatsuser", Jump_DelstatsUserId,ADMFLAG_ROOT, "del stats");
...
public void SQLQueryCallback_DelUserId(Database hDatabase, DBResultSet hResults, const char[] sError, any data) // Обратный вызов
{
	if(sError[0]) // Если произошла ошибка
	{
		LogError("SQLQueryCallback_DelUserId: %s", sError); // Выводим в лог
		return; // Прекращаем выполнение ф-и
	}
}

public Action Jump_DelstatsUserId(int client, int args)
{
	if (args < 1)
	{
		return Plugin_Handled;
	...
	SCmd cmd1;
	cmd1.Create(client, args);
	char arg[50];
	cmd1.GetArg(1, arg, sizeof(arg));
	
	for(int x = 0; x < JUMP_PATTERN; x++){
		char szQuery[256];
		Format(szQuery, sizeof(szQuery),sql_SJS_DELBYUSERID[x],arg);
		g_hDatabase.Query(SQLQueryCallback_DelUserId,szQuery);
	}
	PrintToChat(client, "Deleted %s", arg);
	return Plugin_Handled;
}
